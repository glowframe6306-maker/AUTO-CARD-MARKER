...
const loadPayments = async () => {
  try {
    ...
    if (!isOwner && user.memberProfile?.memberId) {
      const [memberData, paymentStats] = await Promise.all([
        fetcher(
          `${getApiUrl()}/api/payments/member/${user.memberProfile.memberId}`
        ),
        fetcher(`${getApiUrl()}/api/payments/statistics`),
      ]);

      setPayments(Array.isArray(memberData) ? memberData : []);
      setAllPayments(Array.isArray(memberData) ? memberData : []);

      if (!paymentStats || typeof paymentStats !== "object") {
        throw new Error("Invalid payment statistics response.");
      }

      setStats({
        pendingMonths: Number(paymentStats.pendingMonths ?? 0),
        pendingAmount: Number(paymentStats.pendingAmount ?? 0),
        paidMonths: Number(paymentStats.paidMonths ?? 0),
     ...
     */
    const [adminData, paymentStats] = await Promise.all([
...
const statCards = [
  {
    title: "PENDING MONTH",
    value: stats.pendingMonths.toLocaleString(),
    description: "Pending requests",
  },
  {
    title: "PENDING AMOUNT",
    value: `Rs. ${stats.pendingAmount.toLocaleString()}`,
    description: "Pending months × Rs. 200",
  },
  {
    title: "PAID MONTH",
    value: stats.paidMonths.toLocaleString(),
    description: "Completed payments",
  },
  {
    title: "PAID AMOUNT",
    value: `Rs. ${stats.paidAmount.toLocaleString()}`,
    description: "Paid months × Rs. 200",
  },
  {
    title: "FAILED",
    value: stats.failedPayments.toLocaleString(),
    description: "Failed or denied transactions",
  },
];
