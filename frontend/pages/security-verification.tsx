...
import {
  getPermissionState,
  logSecurityVerificationMediaState,
  requestSecurityVerificationStream,
} from "../lib/media";
...
export default function SecurityVerification() {
  ...
  async function createOwnerSelfVerification() {
    setLoading(true);
    setError(null);
    setMessage(null);

    try {
      const response = await authFetch(`${getApiUrl()}/api/verification/self-request`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      });
      ...
      if (!response.ok) {
        throw new Error(data?.error || "Unable to create owner verification record.");
      }

      setSessions((current) => [data, ...current]);
      setMessage(`Owner verification record created successfully. Session #${data.id}`);
      window.dispatchEvent(new Event("verification-session-updated"));
    } catch (err) {
      setError(err instanceof Error ? err.message : "Failed to create owner verification record.");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function startCapture(sessionId: number) {
    ...
    try {
      ...
      logSecurityVerificationMediaState("getUserMedia:startCapture", stream);
      ...
      if (
        ...
        audioTracks.some((track) => !track.enabled || track.readyState !== "live" || track.muted) ||
      ...
      setMediaStream(recordingStream);
      logSecurityVerificationMediaState("before MediaRecorder:startCapture", recordingStream);
    ...
    const supportedMimeType =
      mimeCandidates.find((mimeType) => MediaRecorder.isTypeSupported(mimeType)) || "";

    console.info("[SECURITY-VERIFICATION-AUDIO] MediaRecorder support:startCapture", {
      ...
      selectedMimeType: supportedMimeType || "browser-default",
    ...
    console.info("[SECURITY-VERIFICATION-AUDIO] MediaRecorder created:startCapture", {
      ...
      recorder.ondataavailable = (event) => {
        if (event.data.size > 0) {
          ...
          console.info("[SECURITY-VERIFICATION-AUDIO] MediaRecorder chunk:startCapture", {
            chunkCount,
      ...
      recorder.onstop = () => {
        const recordingType = recorder.mimeType || supportedMimeType || "video/webm";
        const blob = new Blob(chunks, { type: recordingType });
        console.info("[SECURITY-VERIFICATION-AUDIO] MediaRecorder stopped:startCapture", {
          chunkCount,
          totalRecordedBytes,
          blobSize: blob.size,
          blobType: blob.type,
      ...
      setError(
        err instanceof Error && err.message === "Microphone access is required for Security Verification recording."
          ? err.message
          : "Unable to start camera/microphone capture."
      );
    }
  }
    ...
    <div className="space-y-6">
      ...
      <section className="grid gap-6 lg:grid-cols-[1fr_400px]">
        <div className="space-y-6">
          ...
          {user?.isOwner && (
            <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
              <h2 className="text-lg font-semibold text-slate-950">Owner Verification</h2>
              <p className="mt-2 text-sm text-slate-600">
                Create a verification record for your own Owner account.
              </p>

              <button
                type="button"
                onClick={createOwnerSelfVerification}
                disabled={loading}
                className="mt-5 rounded-2xl bg-brand-600 px-5 py-3 text-sm font-semibold text-white transition hover:bg-brand-700 disabled:opacity-50"
              >
                {loading ? "Creating..." : "START OWNER VERIFICATION"}
              </button>

              <div className="mt-8 border-t border-slate-200 pt-6">
                <h2 className="text-lg font-semibold text-slate-950">Request Verification</h2>
                <p className="mt-2 text-sm text-slate-600">
                  Create a secure verification request for another account.
                </p>
