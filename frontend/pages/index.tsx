...
import { authFetch, getApiUrl, setAuthToken, clearAuthToken } from "../lib/api";
import { getPermissionState, initMediaStream } from "../lib/media";
export default function Home() {
  useEffect(() => {
    ...
    const requestNativePermissions = async () => {
      ...
      try {
        ...
        // Permission only. Release devices immediately.
  ...
  // no camera required for registration; keep component cleanup minimal
      ...
      <div className="mx-auto flex max-w-6xl flex-col gap-12 lg:flex-row lg:items-center lg:justify-between">
        ...
        <section className="w-full max-w-md rounded-[32px] border border-slate-200 bg-white p-10 shadow-xl">
          <div className="mb-8 space-y-3">
            <div className="auto-card-marking-signin-logo-wrap">
              <img
                src="/assets/logo.png"
                alt="AUTO CARD MARKING"
                className="auto-card-marking-signin-logo"
              />
            </div>            
<p className="text-sm uppercase tracking-[0.24em] text-brand-600">Secure Sign In</p>
  ...
  <form className="space-y-5" onSubmit={handleSubmit}>
    ...
    >
      {loading ? "Signing in..." : "Sign in"}
            ...
            <form
              ...
              onSubmit={async (e) => {
                ...
                if (!name || !rcStudentId || !email || !dob || !regPassword || !confirmPassword) {
                ...
                try {
                  ...
                  const url = `${getApiUrl()}/api/registrations`;
                  ...
                  if (!res.ok) {
                    throw new Error(
                      ...
                      body?.error ||
                      raw ||
                      `Registration failed with HTTP ${res.status}`
                    );
                  }
                  ...
                  setRegMessage(
                    body?.message ||
                    "YOUR REGISTER DETAILS WAS SENT TO MANAGEMENT TEAM."
                  );
            ...
            >
              <label className="block text-sm font-medium text-slate-700">
                ...
                <input
                  ...
                  className="mt-1 w-full rounded-2xl border border-slate-300 px-4 py-3 bg-slate-50 text-sm"
                />
              </label>
              ...
              <label className="block text-sm font-medium text-slate-700">
                ...
                <input
                  ...
                  type="text"
                  placeholder="Enter your RC Student ID"
                  className="mt-1 w-full rounded-2xl border border-slate-300 px-4 py-3 bg-slate-50 text-sm"
                />
              </label>
              ...
              <div className="flex items-center gap-3">
                <button
                  type="submit"
                  disabled={regLoading}
                  className="rounded-2xl bg-brand-600 px-5 py-3 text-sm text-white"
                >
      ...
      <style jsx>{`
        .auto-card-marking-signin-logo-wrap {
          display: flex;
        ...
        .auto-card-marking-signin-logo {
          ...
          animation:
            autoCardMarkingLogoIntro 700ms ease-out both,
            autoCardMarkingLogoFloat 4s ease-in-out 800ms infinite;
        }

        @keyframes autoCardMarkingLogoIntro {
          0% {
            opacity: 0;
        ...
        @keyframes autoCardMarkingLogoFloat {
          0%,
          100% {
