...
router.get("/:receiptNumber/pdf", async (req, res) => {
    try {
        ...
        const safeFileName = receipt.receiptNumber.replace(/[^a-zA-Z0-9-_]/g, "_");
        ...
        const monthText = receipt.month >= 1 && receipt.month <= 12
        ...
        if (receipt.payment) {
            ...
            doc
                .moveDown(0.4)
                .font("Helvetica")
                .text(`Payment Date: ${paymentDate.toLocaleDateString("en-GB")}`)
                .text(`Payment Amount: Rs. ${receipt.payment.paymentAmount.toLocaleString()}`)
        ...
        doc
            .fontSize(10)
            .text(`Issued By: ${receipt.issuedBy?.fullName ?? "System Owner"}`);
        doc.moveDown(3);
        /*
         * FOOTER
         */
        doc
            .fontSize(9)
            .fillColor("#666666")
            .text("This is a system-generated payment receipt.", {
            align: "center",
        });
        doc.end();
    }
