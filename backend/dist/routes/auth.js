...
router.patch("/me", authMiddleware_1.authenticate, async (req, res) => {
    try {
        if (!req.user) {
            return res.status(401).json({ error: "Unauthorized." });
        }
        ...
        const roleAssignments = updatedUser.roles;
        const roles = roleAssignments.map((assignment) => assignment.role.name);
        const permissions = Array.from(new Set(roleAssignments.flatMap((assignment) => assignment.role.permissions?.map((permission) => permission.permission.name) || [])));
        return res.json({
            success: true,
...
router.post("/change-password", authMiddleware_1.authenticate, async (req, res) => {
    try {
        if (!req.user) {
            return res.status(401).json({ error: "Unauthorized" });
        }
        ...
        const passwordValid = await (0, auth_1.verifyPassword)(currentPassword, user.passwordHash);
        if (!passwordValid) {
