  ...
  try {
    ...
    if (requestedStatus === "PAID") {
      if (!existingPayment) {
        ...
        const memberProfile = await prisma.memberProfile.findUnique({ where: { id: memberId }, include: { user: true } });
        if (memberProfile && !memberProfile.user.isOwner) {
          const paymentEventKey = `MONTHLY_PAYMENT:${memberProfile.userId}:${month.academicYearId}:${month.month}:${payment.id}`;
          await prisma.pointTransaction.upsert({
            where: { eventKey: paymentEventKey },
            update: {},
            create: {
              ...
              description: `Monthly payment completed for month ${month.month} (payment #${payment.id}).`,
            },
          });
        }
      }
