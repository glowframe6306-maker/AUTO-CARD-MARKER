...
router.delete("/:memberId", requirePermission("manage_members"), async (req: AuthorizedRequest, res) => {
  ...
  try {
    await prisma.$transaction(async (tx) => {
      ...
      const paymentUploads = paymentIds.length > 0
        ? await tx.cardUpload.findMany({
            where: {
              paymentId: {
                in: paymentIds
              }
        ...
        : [];
      const paymentUploadIds = paymentUploads.map((item) => item.id);
      ...
      if (paymentIds.length > 0) {
        ...
        const paymentAuditLogs = await tx.auditLog.findMany({
          where: {
            paymentId: {
              in: paymentIds
            }
        ...
        const paymentAuditLogIds = paymentAuditLogs.map((item) => item.id);

        if (paymentAuditLogIds.length > 0) {
          await tx.approvalRequest.updateMany({
            where: {
              auditLogId: {
                in: paymentAuditLogIds
              }
            },
            data: {
              auditLogId: null
      ...
      /*
       * Delete chat records owned by this user. User-owned participant,
       * reaction, star, pin, delivery, and notification rows are removed
       * explicitly so the User delete does not rely on database cascades.
       */
      ...
      const actorAuditLogs = await tx.auditLog.findMany({
        where: {
          actorId: userId
        },
        select: {
      ...
      const actorAuditLogIds = actorAuditLogs.map((item) => item.id);

      if (actorAuditLogIds.length > 0) {
        await tx.approvalRequest.updateMany({
          where: {
            auditLogId: {
              in: actorAuditLogIds
      ...
      /*
       * Pending registrations are not connected by a foreign key, but a
       * matching pending registration belongs to the deleted member.
       */
      await tx.pendingRegistration.deleteMany({
        where: {
          rcStudentId: memberId
        }
      });
    ...
    return res.status(isForeignKeyConflict ? 409 : 500).json({
      error: isForeignKeyConflict
        ? "Member deletion could not be completed because related records still exist."
        : "Member deletion failed due to a database error.",
      details:
