  ...
  try {
    ...
    const announcement = await prisma.announcement.create({
      data: {
        ...
        targetGrade:
          finalTargetType === "GRADE" ? String(targetGrade).trim() : null,
        targetMemberId:
          finalTargetType === "MEMBER"
 ...
 * OWNER ONLY
 */
router.put("/:id", requireOwner, async (req: AuthorizedRequest, res) => {
  try {
    const id = Number(req.params.id);
 ...
 * OWNER ONLY
 */
router.patch("/:id/pin", requireOwner, async (req: AuthorizedRequest, res) => {
  try {
    const id = Number(req.params.id);
