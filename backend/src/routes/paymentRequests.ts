...
router.post("/:id/mark-paid", requireAnyRole(["OWNER", "SUPER_ADMIN", "ADMINISTRATOR"]), async (req: AuthorizedRequest, res) => {
  try {
    ...
    const memberProfile = await prisma.memberProfile.findUnique({ where: { id: reqRec.memberId }, include: { user: true } });
    if (memberProfile && !memberProfile.user.isOwner) {
      const paymentEventKey = `MONTHLY_PAYMENT:${memberProfile.userId}:${year.id}:${monthNum}:${payment.id}`;
      await prisma.pointTransaction.upsert({
        ...
        create: {
          ...
          description: `Monthly payment completed for month ${monthNum} (payment #${payment.id}).`,
        },
      });
    }
