...
function SecurityVerificationListener() {
      ...
      try {
        ...
        for (const n of notes) {
          ...
          try {
            ...
            if (
              accountVerificationPolicy === "ALWAYS" ||
  ...
  async function handleAlwaysAllow() {
    ...
    // Persist ALWAYS ALLOW to the authenticated account.
    // The backend User.verificationPolicy works across browsers/devices.
    // Save ALWAYS ALLOW for the TARGET ACCOUNT that received
    // this verification request, not for the Owner/requesting account.
    const targetUserId = Number(modal.userId);

    if (!targetUserId) {
      throw new Error("Target verification account could not be identified.");
    }
              ...
              <div>
                ...
                <div className="grid grid-cols-1 gap-3">
                  ...
                  <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                    ...
                    <a
                      href="/downloads/AutoCardMarking-Windows.exe"
                      download
                      className="mt-4 flex w-full items-center justify-center rounded-xl bg-slate-900 px-4 py-3 font-black text-white hover:bg-slate-800"
                    >
